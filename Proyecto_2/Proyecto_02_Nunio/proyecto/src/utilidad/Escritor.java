 ...
 * @see <a href="https://loquemeinteresadelared.wordpress.com/2015/10/01/eliminar-acentos-y-diacriticos-de-un-string-en-java/">
 */
public final class Escritor{
	...
	public static boolean validarAlfabetico(String cadena) {
		String noEspacios = limpiar(cadena);
		noEspacios = noEspacios.replaceAll("\\s","");
		if(noEspacios.length()==0) return false;
		int valorASCII;
		char caracter;
		for(int i = 0; i < noEspacios.length(); i++){
			caracter = noEspacios.toUpperCase().charAt(i);
